...
import {ReservationButtonSkeleton, ReservationDescriptionSkeleton} from "./ReservationButtonSkeleton";
    ...
    <div className='w-full bg-white relative rounded-lg shadow dark:border md:mt-0 sm:max-w-md dark:bg-gray-800 dark:border-gray-700 p-6 space-y-10 md:space-y-12 sm:p-8'>
      {isLoading ? (
        ...
        <Suspense fallback={<ReservationButtonSkeleton />}>
          <MyReservation id={data.reservationId as any} onClickHandler={on} />
        </Suspense>
      ) : (
      ...
      <div className='inline-flex items-center justify-center w-full'>
        ...
        {isLoading ? (
           ...
           <div className="w-full flex flex-col justify-center items-center">
                  <KeywordHighlight before={"다른 사람이"} keyword={"QR코드"} after={"를 스캔하면"} rest={"예약에 참가할 수 있습니다."}></KeywordHighlight>
            <Image src={'/temp.png'} alt={"device mock"} width={200} height={300}/>
           </div>
        ) : (
            <div className={"space-y-6"}>
                <KeywordHighlight keyword={"+버튼 또는 강의실"} after={"을 눌러"} rest={"예약을 생성하세요."}></KeywordHighlight>
                <Rooms />
            </div>
        )}
