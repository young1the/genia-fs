...
import { deEntrantReservation } from "@/lib/api/reservation/method";
import toast from "react-hot-toast";

const CancelModal = ({ reservationData, off }: ModalProps) => {
  const queryClient = useQueryClient();
  const { mutate } = useMutation(
    async () => {
      return toast.promise(
        deEntrantReservation(reservationData.reservationCode as any),
        {
          loading: "기다려주세요...",
          success: "완료",
          error: "Error",
